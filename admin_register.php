...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validation
    if (empty($username) || empty($email) || empty($password) || empty($confirm_password) || empty($phone) || empty($admin_code)) {
        $error = 'Please fill in all required fields';
    } elseif ($password !== $confirm_password) {
        $error = 'Passwords do not match';
    } elseif (strlen($password) < 8) {
        $error = 'Password must be at least 8 characters long';
    } elseif ($admin_code !== 'ADMIN2024') { // 这里使用一个简单的注册码，实际应用中应该更安全
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (username, email, password, phone, address, role) VALUES (?, ?, ?, ?, ?, 'admin')");
...
<head>
    ...
    <title>Admin Registration - TriHealth Mart</title>
